...
/**
 * This class summarizes some code fragments to demonstrate the usage of HTTP APIs with silvertunnel.org Netlib.
 * 
 * This class is directly visible from some Wiki pages and part of the silvertunnel.org Netlib documentation. The code
 * fragments are collected here instead and not in the Wiki to ensure that they are always valid/correct and compile
 * against the current version of the API. (The author saw wrong/non compilable examples in other projects documentation
 * too often - therefore we document it here)
 * 
 * @author hapke
 */
public class NetlibHttpUsageExamplesRemoteTest {
	 ...
	 * 
	 * More documentation: http://sourceforge.net/apps/trac/silvertunnel/wiki/NetlibHttpUtil
	 */
	@Test
	public void simple_HTTP_GET_request_with_HttpUtil() {
		try {
			...
			byte[] responseBody = HttpUtil.getInstance().get(lowerNetLayer, httpServerNetAddress, pathOnHttpServer,
					timeoutInMs);
	...
	/**
	 * Example: HTTP request using Adapter URL to load http://httptest.silvertunnel.org/httptest/bigtest.jsp?id=example2
	 * 
	 * This example does not change the global settings of java.net.URL and is preferred over the next example (
	...
	/**
	 * Example: HTTP request using Adapter URL to load http://httptest.silvertunnel.org/httptest/bigtest.jsp?id=example3
	 * 
	 * This example changes the global settings of java.net.URL and can have side effects to code in the same JVM. The
	 * way shown here should only be used if necessary. Otherwise, the previous example (
	 * execute_HTTP_GET_with_Adapter_URL_with_global_reconfiguration_of_class_URL ) if preferred.
	 * 
	 * More documentation: http://sourceforge.net/apps/trac/silvertunnel/wiki/NetlibHttpUtil
	 */
